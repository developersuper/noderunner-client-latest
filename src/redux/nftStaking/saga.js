...
export function* getStakedCardsCount() {
  yield takeLatest(actions.GET_STAKED_CARDS_COUNT, function* ({ payload }) {
    const { selectedCardIds } = payload;
    const stakedAllCards = JSON.parse(localStorage.getItem('stakedToekns'));
    if (!stakedAllCards) {
      ...
      if (res.status === 200) {
        ...
        for (let i = 0; i < selectedCardIds.length; i ++){
          const cardIndex = cards.findIndex(
            (e) => Number(e.id) === Number(selectedCardIds[i])
          );
          cards[cardIndex].staked = 1;
        }
        localStorage.setItem('stakedToekns', JSON.stringify(cards));
      }
    } else {
      const cards = [...stakedAllCards];
      const cardIndex = cards.findIndex(
        (e) => Number(e.id) === Number(selectedCardIds[0])
      );
      cards[cardIndex].staked = 1;
      localStorage.setItem('stakedToekns', JSON.stringify(cards));
    }  
...
export function* getStakedCards() {
  yield takeLatest(actions.GET_STAKED_CARDS, function* () {
    const web3 = yield call(getWeb3);
    const getStakingNFT = getStakedNFTInstance(web3);
    //for round1
    const accounts = yield call(web3.eth.getAccounts);
    const myStakedCards = [];
    
    for (let i = 0; i < myStakedCards.length + 1; i++) {
      const cnt = yield call(getStakedCountByTokenIdAsync, getStakingNFT.instance, accounts[0], i);
      if (cnt[0] > 0) {
        myStakedCards.push(cnt.tokenId);
      }
    }
